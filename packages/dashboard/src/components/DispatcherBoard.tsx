...
const LANES: Array<{ id: Lane; title: string; blurb: string; accent: string }> = [
  {
    id: 'machine',
    title: 'Copilot agent',
  ...
  return (
    <span className="flex gap-0.5" title={`${score}/4 agent-readiness`}>
      {[0, 1, 2, 3].map((i) => (
        <span
          key={i}
          className={`h-1.5 w-3 rounded-full ${i < score ? 'bg-good' : 'bg-edge'}`}
        />
      ))}
    </span>
  );
...
function Card({
  ...
  return (
    <li className="group rounded-lg border border-edge bg-panel p-3 transition hover:border-slate-500">
      <div className="flex items-start justify-between gap-2">
        <a
          href={card.url}
          target="_blank"
          rel="noreferrer"
          className="text-sm font-medium leading-snug text-slate-100 hover:text-machine hover:underline"
        >
      ...
      <button
        ...
        className="mt-2 w-full rounded border border-edge px-2 py-1 text-[11px] text-muted opacity-0 transition group-hover:opacity-100 hover:border-slate-400 hover:text-slate-200 disabled:opacity-50"
      >
...
export function DispatcherBoard({ cards, repo }: { cards: BoardCard[]; repo: string }) {
  ...
  return (
    <section>
      ...
      {note && (
        <div className="mb-4 rounded border border-edge bg-panel px-3 py-2 text-sm text-slate-300">
          {note}
        </div>
      )}
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {LANES.map((lane) => {
          ...
          return (
            <div key={lane.id} className={`rounded-xl border-t-2 ${lane.accent} bg-panel/40 p-3`}>
